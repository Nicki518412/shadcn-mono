...
test.describe("低权限授权边界", () => {
  test("访客看不到管理菜单，深链不可达，直接写 API 返回 403", async ({ page, request }) => {
    ...
    const guestRoleRes = await request.get("http://localhost:3001/api/roles?keyword=GUEST", {
    ...
    const guestLogin = await request.post("http://localhost:3001/api/auth/login", {
    ...
    const forbidden = await request.post("http://localhost:3001/api/users", {
